...
public class NagiosCfgJavaValidator extends AbstractNagiosCfgJavaValidator {
	...
	public void checkAttributeUniqueness(ConfigObject cfgObj) {
		EList<ObjectContent> attrs = cfgObj.getAttr();
		HashSet<String> found = new HashSet<String>();
		int i = 0;
		for (ObjectContent content : attrs) {
			if (content instanceof Attribute) {
				...
				if (found.contains(clazz)) {
					String cName = attribute.getToken() != null
							&& attribute.getToken() != "" ? attribute
							.getToken() : clazz;
					error("Duplicate entry of '" + cName + "' found", cfgObj,
							attribute.eContainingFeature(), i);
